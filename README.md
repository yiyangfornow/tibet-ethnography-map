...
- 地图默认按文化区域着色，不再提供颜色模式切换。
- 新增 `data/cultural_regions.geojson`：用半透明色块呈现卫藏、阿里、康区、安多、跨喜马拉雅、不丹—锡金和流动交汇节点。
- 新增 `data/focus_mask.geojson`：弱化研究范围外的世界底图，帮助用户先聚焦藏文化区域。
- 默认页面显示大文化区域标题。
- 修复搜索体验：加入别名字段，例如“科加寺”会匹配“科迦寺”，“扎达”会匹配“札达”。
- 补入 V3–V6 批次重点点位，并为主要点位加入公开 Wikimedia Commons 图片链接。



## V3 更新摘要

- 地图默认按文化区域着色：新增 `data/region_areas.geojson` 和 `data/region_labels.geojson`。
- 外围非研究范围使用灰色遮罩；区域名称直接显示在地图上。
- 移除“地图颜色”下拉选择，不再让用户切换着色逻辑。
- 关闭点位 cluster，避免默认页面只看到数字而不知道文化区域。
- 修复搜索体验：输入后会自动筛选并定位；加入“科加/科迦、扎达/札达、冈仁波钦/冈仁波齐”等别名。
- 补充 V3–V6 重点点位说明：卫藏、康区、安多、跨喜马拉雅、不丹—锡金与流动节点。
- 为重点点位加入 Wikimedia Commons 等公开图片链接；正式上线前仍需逐张核对授权与署名。
